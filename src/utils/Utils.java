...
public class Utils {
	...
	/**
	 * Tìm document trong index theo DOCNO (external ID) Hàm trả về DOCID (internal
	 * ID) của document hoặc -1 nếu không tìm thấy
	 *
	 * @param index
	 *            index reader
	 * @param docnoField
	 *            Tên field được sử dụng để lưu DOCNO (external document IDs)
	 * @param docno
	 *            DOCNO (external ID) cần tìm
	 * @return DOCNO (external ID) của document trong index hoặc -1 nếu không tìm
	 *         thấy
	 * @throws IOException
	 */
	 ...
	 *            Tên field được sử dụng để lưu DOCNO (external document IDs)
	 * @param docid
	 *            DOCID (internal ID) của document
